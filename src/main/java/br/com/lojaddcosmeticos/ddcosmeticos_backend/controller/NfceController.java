...
@Tag(name = "Fiscal NFC-e", description = "Emissão, Impressão e Status de Cupom Fiscal Eletrónico (Modelo 65)")
public class NfceController {
    ...
    public ResponseEntity<String> verificarStatusSefazNfce() {
        try {
            log.info("A iniciar ping ao servidor da SEFAZ (NFC-e)...");
            return ResponseEntity.ok(nfceService.consultarStatusSefaz());
        } catch (Exception e) {
    ...
    @Operation(summary = "Imprimir Cupom (PDF - Térmica)", description = "Gera o PDF do DANFE NFC-e (80mm) com download automático.")
    public ResponseEntity<byte[]> imprimirCupom(@PathVariable Long idVenda) {
        byte[] pdf = impressaoService.gerarCupomNfce(idVenda);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=cupom_" + idVenda + ".pdf")
                .contentType(MediaType.APPLICATION_PDF)
                .body(pdf);
    }

    @GetMapping(value = "/{idVenda}/pdf", produces = MediaType.APPLICATION_PDF_VALUE)
    @Operation(summary = "Visualizar Cupom (PDF via Frontend)", description = "Rota otimizada para o frontend abrir o PDF em iframe ou nova aba (Inline).")
    public ResponseEntity<byte[]> baixarCupomPdf(@PathVariable Long idVenda) {
        byte[] pdfBytes = impressaoService.gerarCupomNfce(idVenda);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=CupomFiscal_" + idVenda + ".pdf")
                .contentType(MediaType.APPLICATION_PDF)
                .body(pdfBytes);
    }

    @PostMapping("/retransmitir/{idVenda}")
    @Operation(summary = "Retransmitir NFC-e (Contingência)", description = "Tenta reemitir uma nota fiscal que ficou em contingência ou foi rejeitada pela SEFAZ.")
    public ResponseEntity<?> retransmitirNfce(@PathVariable Long idVenda) {
        try {
            Optional<Venda> vendaOpt = vendaRepository.findById(idVenda);
            if (vendaOpt.isEmpty()) { return ResponseEntity.notFound().build(); }

            NfceResponseDTO resposta = nfceService.emitirNfce(vendaOpt.get());
