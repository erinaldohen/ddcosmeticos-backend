...
public class EstoqueFinanceiroIntegrationTest {
    ...
    @Test
    @DisplayName("PIX: Deve gerar conta e BAIXAR automaticamente")
    @WithMockUser(username = "gerente", roles = {"GERENTE"})
    public void testeCompraAvistaPix() {
        executarTestePagamentoImediato(FormaDePagamento.PIX, "789111111", "NF-PIX");
    }

    @Test
    @DisplayName("DINHEIRO: Deve gerar conta e BAIXAR automaticamente")
    @WithMockUser(username = "gerente", roles = {"GERENTE"})
    public void testeCompraAvistaDinheiro() {
        executarTestePagamentoImediato(FormaDePagamento.DINHEIRO, "789222222", "NF-DIN");
    }

    @Test
    @DisplayName("DÉBITO: Deve gerar conta e BAIXAR automaticamente")
    @WithMockUser(username = "gerente", roles = {"GERENTE"})
    public void testeCompraAvistaDebito() {
        executarTestePagamentoImediato(FormaDePagamento.DEBITO, "789333333", "NF-DEB");
    }
    ...
    @Test
    @DisplayName("BOLETO (A Prazo): Deve gerar conta PENDENTE")
    @WithMockUser(username = "gerente", roles = {"GERENTE"})
    public void testeCompraBoleto() {
        criarProduto("789444444", new BigDecimal("100.00"));

        EstoqueRequestDTO dto = criarDtoBase("789444444", "NF-BOL");
    ...
    private EstoqueRequestDTO criarDtoBase(String codigo, String nf) {
        ...
        dto.setFornecedorCnpj("11111111000111");
